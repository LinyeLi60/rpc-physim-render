...
class Camera:
    ...
    def __init__(self, camIntrinsic, camExtrinsic, numViews):
        ...
        sensor_width_in_mm = self.camIntrinsic[1][1] * self.camIntrinsic[0][2] / (
                self.camIntrinsic[0][0] * self.camIntrinsic[1][2])
        ...
        cam_ob.data.lens = f_in_mm    # 1.290509033333333
        cam_ob.data.lens_unit = 'MILLIMETERS'
        cam_ob.data.sensor_width = sensor_width_in_mm    # 1.3333335959343278

        """
cam_ob = bpy.context.scene.objects['Camera']
cam_ob.location = [0, 0, 2]
cam_ob.rotation_mode = 'QUATERNION'
cam_ob.rotation_quaternion = [1, 0, 0, 0]

cam_ob.data.type = 'PERSP'

cam_ob.data.sensor_width = 1.3333335959343278
cam_ob.data.lens_unit = 'MILLIMETERS'
cam_ob.data.lens = 1.290509033333333
cam_ob.data.sensor_width = 1.3333335959343278
bpy.context.scene.camera = cam_ob
bpy.context.scene.update()
        """
        print("相机内参", f_in_mm, sensor_width_in_mm)
        bpy.context.scene.camera = cam_ob
        bpy.context.scene.update()

    def placeCamera(self, view):
        bpy.data.objects['Camera'].location = (
            self.camExtrinsic[view][0], self.camExtrinsic[view][1], self.camExtrinsic[view][2])
    ...
    def write_bounds_2d(self, filepath, me_ob, index):
        with open(filepath, "a+") as file:
            ...
            if x > bpy.context.scene.render.resolution_x or \
                ...
                # 保存 物体类别, x, y, w, h
                file.write("%s,%i,%i,%i,%i,%f\n" % (me_ob.name, x, y, width, height, me_ob.matrix_world.translation[0]))
