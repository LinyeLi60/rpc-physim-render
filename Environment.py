...
class Board:
    def __init__(self, shape_file):
        bpy.ops.import_scene.obj(filepath=shape_file)
        object_instance = bpy.data.objects["Board"]
        object_instance.location = [0, 0, 0]
        object_instance.rotation_mode = 'QUATERNION'
        object_instance.rotation_quaternion = [0.7, 0.7, 0, 0]

        bpy.context.scene.objects.active = bpy.context.scene.objects["Board"]
        bpy.ops.rigidbody.object_add(type='ACTIVE')    # 活动项
        bpy.ops.object.modifier_add(type='COLLISION')
        object_instance.rigid_body.enabled = False
        object_instance.rigid_body.use_margin = True
        object_instance.rigid_body.collision_margin = 0.01
...
class Light:
    ...
    GlobalEnergyLowbound = 1
    GlobalEnergyHighbound = 2

    def __init__(self):
        bpy.ops.object.select_by_type(type='LAMP')
        bpy.ops.object.delete(use_global=False)
        bpy.context.scene.world.light_settings.use_environment_light = True
        bpy.context.scene.world.light_settings.environment_energy = random.uniform(self.GlobalEnergyLowbound,
                                                                                   self.GlobalEnergyHighbound)
    ...
    def placePointLight(self, light_range_x, light_range_y, light_range_z):
        ...
        # 强度
        bpy.data.objects['Point'].data.energy = random.randint(1, 5)
        # 颜色
